...
namespace com::wardenre::ModerationX::DataBase::Wrapper {
    ...
    SQLiteWrapper::SQLiteWrapper(const std::string& dbPath) : db_(nullptr) {
        ...
        if (rc != SQLITE_OK) {
            ...
            ModerationX::getInstance().getLogger().error("Failed to open database: {}", lastError_);
            sqlite3_close(db_);
            db_ = nullptr;
            return;
        }
    ...
    bool SQLiteWrapper::execute(const std::string& sql, const std::vector<std::string>& params) {
        ...
        if (!bindParameters(stmt, params)) { sqlite3_finalize(stmt); return false; }
        int  rc      = sqlite3_step(stmt);
        bool success = (rc == SQLITE_DONE);
        if (!success) lastError_ = sqlite3_errmsg(db_);
    ...
    std::vector<std::vector<std::string>> SQLiteWrapper::query(const std::string& sql) {
        ...
        sqlite3_stmt* stmt = nullptr;
        if (!prepareStatement(sql, &stmt)) return result;
        ...
        const std::vector<std::string>& params)
    {
        ...
        sqlite3_stmt* stmt = nullptr;
        if (!prepareStatement(sql, &stmt)) return result;
        if (!bindParameters(stmt, params)) { sqlite3_finalize(stmt); return result; }
        int columnCount = sqlite3_column_count(stmt);
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            std::vector<std::string> row;
            row.reserve(columnCount);
    ...
    bool SQLiteWrapper::beginTransaction()    { return execute("BEGIN TRANSACTION;"); }
    bool SQLiteWrapper::commitTransaction()   { return execute("COMMIT;"); }
    bool SQLiteWrapper::rollbackTransaction() { return execute("ROLLBACK;"); }

    std::string SQLiteWrapper::getLastError()    const { return lastError_; }
    int64_t     SQLiteWrapper::getLastInsertId() const { return db_ ? sqlite3_last_insert_rowid(db_) : -1; }
    bool        SQLiteWrapper::isOpen()          const { return db_ != nullptr; }
